...
- `Ctrl-?` 打开全局快捷键参考框，查看有效配置、全部 focus 操作集和 commit/remote 等 modal 操作集。
...
| `Ctrl-?` / `?` | 从任意普通 focus 打开全局快捷键参考框；`?` 是无法区分 Ctrl-? 与 Backspace 的终端 fallback；`↑/↓`、PageUp/PageDown、Home/End 滚动，`Ctrl-?` / `?` / `Enter` / `Esc` / `q` 关闭 |
...
| `↑` / `k`, `↓` / `j` | 移动选择或滚动 diff |
| `←` / `h`, `→` / `l` | 在分支/提交浏览主链中按 `Branches → Commits → Commit → Diff` 层级移动；到达双栏边界后平移界面，旧右栏成为新左栏，不会循环回起点 |
...
| Commits | `→` / `l` | 将 `Branches｜Commits` 平移为 `Commits｜Commit`；Commits 成为左栏并保持焦点 |
...
第二行显示日期、作者与 Git tags；平移到 `Commits｜Commit` 后，Commits 作为左栏恢复为原有
单行紧凑样式，以保留更多纵向空间。
...
| 焦点 | Key | 操作 |
|---|---|---|
| Commits | `↑` / `k`, `↓` / `j` | 选择 commit 并自动刷新右侧详情，焦点保持在左侧 Commits |
| Commits | `Ctrl-C` → `h` / `i` / `m` | 复制当前/多选 hash、当前 info 或完整 message |
| Commits / Commit | `←` / `h`, `→` / `l` | 在 Commits 与完整 Commit 栏（metadata + files）间移动；越过边界时返回 Overview 或进入 File Diff |
...
| Key | 操作 |
|---|---|
| `←` / `h`, `→` / `l` | 在完整 Commit 栏和 Diff 间移动；从左栏继续向左返回 `Commits｜Commit` |
