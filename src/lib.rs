//! Composition root for the Data Driven Pitui runtime.
...
impl App {
        ...
        let (log_sink, startup_notice): (
            ...
            match pitui_git::logging::JsonlGitOperationLogSink::open(sink_config) {
                ...
                Err(error) if config.fail_on_open_error => {
                    return Err(AppError::GitLogOpen {
                        path: config.path,
                        error,
                    });
                }
    ...
    fn open_from_with_log_sink(
        ...
        if !contract_errors.is_empty() {
            return Err(AppError::BuiltinContract(contract_errors));
        }
        ...
        if !violations.is_empty() {
            return Err(AppError::Invariant(violations));
        }
...
pub fn run_terminal(mut app: App) -> io::Result<()> {
