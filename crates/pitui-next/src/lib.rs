...
mod tests {
    ...
    use pitui_data::{
        ...
        ContextStack, DatasetChildren, DatasetCursor, DatasetIdentity, DatasetKey,
        DatasetSelection, FieldId, GitOperationLogEntryMetadata, GitOperationStatus,
    ...
    fn failed_commit_restores_changes_then_opens_a_redacted_notice_context() {
        ...
        let failed_commit = app
            ...
            .find(|entry| {
                entry.operation == "commit" && entry.status == GitOperationStatus::Failure
            })
    ...
    fn injected_jsonl_sink_and_session_log_receive_the_same_git_results() {
        ...
        let sink = pitui_git::logging::JsonlGitOperationLogSink::open(
            pitui_git::logging::JsonlGitLogConfig {
                path: path.clone(),
    ...
    fn commits_selection_is_the_only_cherry_pick_source_and_replays_oldest_first() {
        ...
        let selected_hashes = selected
            .iter()
            .filter_map(|entity| {
                match &app.runtime().world().get::<DatasetKey>(*entity).unwrap().0 {
