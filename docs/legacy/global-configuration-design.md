...
> Command Registry 重写。Next 当前实现与缺口见
...
2. 新增 View、FocusKind 或二级快捷键时，需要同时修改多个 `match`。
...
struct GlobalConfigFile {
    ...
    diff: DiffConfigFile,
    views: Map<ViewId, ViewConfigFile>,
...
配置不得直接引用 Rust enum variant，也不得把页面/焦点判断写进 TOML。
程序内部维护稳定的 `OperationSpec`。`OperationId` 使用 `#[repr(usize)]`，其 discriminant
直接索引 `OPERATION_SPECS`；`invoke` 是可调用函数指针，因此它是 Rust 中对应 C++
...
struct OperationSpec {
    ...
    contexts: u16,             // FocusKind bit set
    invoke: OperationHandler,
}
```

正常模式使用九个语义 `FocusKind`：

```text
repository       branch       commit
file             diff         reflog
changes          changes-diff remote
```

`OperationMount::Global` 始终挂载；`OperationMount::Focus` 必须先命中当前 context bit，再
调用 `invoke` 检查 selection、pending job、scroll boundary 等动态条件。这样即使同一 View
或同一份 Commit 数据处于不同 focus，也只能响应当前语义层级的操作表。
...
KeyEvent
  ...
  -> OperationId
  -> OPERATION_SPECS[OperationId as usize].invoke
  -> Action
...
2. 打开帮助前来源 focus 对应的唯一 `FocusKind` command table。
...
Esc 取消。命令通过 `PROMPT_OPERATION_SPECS` 可调用表解析，而不是散落在 input 分支中。
...
| `trace` | operation id、semantic focus 和 job 调度细节，不记录文本输入内容 |
...
src/app/command.rs       OperationId registry、actionability 与默认 presentation
```
...
- `OPERATION_SPECS[OperationId as usize]` 与 enum 全量、顺序一致，command id 唯一且可 round-trip。
...
- `h` 从各 Normal focus 打开当前-focus 参考框，只列 Global 与来源 `FocusKind` 的有效
...
Phase 9  basic per-View layout/commit fields/scoped operation bindings
```
...
Schema v1 已包含基础 Global → View → Fields / Operations 层：六个 View 可分别配置左栏
宽度、commit density、作者/日期/tag 显示，以及 scoped operation bindings。完整的
Global → View → Panel → Fields / Operations 目标、继承优先级、安全边界和尚未实现的 panel
细粒度选项见 [`view-configuration-design.md`](view-configuration-design.md)。解析器会严格
拒绝未知 View、Operation 和尚未开放的字段，而不是静默忽略。
