...
`PendingJobKind::FileDiff` 携带 `focus_diff` intent：只有显式 `Enter/OpenSelectedFileDiff` 的响应可以把 focus 设为 `DiffView`；由 `↑/↓/PageUp/PageDown/Home/End/n/p` 触发的 diff refresh 必须保留响应到达时的 focus，异步 response 不得抢走左侧文件列表焦点。PageUp/PageDown 直接计算最终 selection，不为中间 9 个文件排队无用 job。
...
Space expand | Enter file diff | Home/End | PgUp/PgDn | y queue | Esc back
...
Global normal mode:
Ctrl+G changes | Ctrl+R refresh（所有主 screen 都放在 hotkey bar 最前面）
```
...
- Ctrl+C-prefixed CommitCopy secondary shortcut palette
