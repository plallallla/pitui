...
fn resolve_command(command: OperationId, app: &AppState) -> Option<Action> {
    let spec = &OPERATION_SPECS[command as usize];
    if spec.mount == CommandMount::Focus {
        let context = FocusKind::from_view(app.screen, app.focus)?;
...
`FocusKind` 的十张 focus 表与 §6.4、§6.5、§7.4、§7.5、§8.4、§8.5、Reflog、
ChangesTree、ChangesDiff、Remotes 操作集一一对应。`ResolvedKeymap::resolve` 和 footer 都先
遍历同一 `OPERATION_SPECS`，再调用同一函数指针；Renderer 不维护另一份按键字符串。
...
可接受命令由 `PROMPT_OPERATION_SPECS` 可调用表维护，表项包含 name、description、operation id
...
帮助内容只由有效 keymap 的 Global 表与来源 focus 对应的唯一 `FocusKind` 表生成；
...
- callable OperationSpec / modal input jump tables, `h` current-focus shortcut reference, and Ctrl+Backtick command prompt
...
1. 建立稳定的 `OperationId + OperationSpec` 可调用跳表；`OperationId as usize` 直接索引包含
...
8. Normal 命令按九个 `FocusKind` 精确挂载：Commits 使用 commit copy，文件列使用
   file copy，DiffView 不挂载 copy；modal 输入另由 `MODE_KEY_TABLES` 独占。
9. `h` 打开当前-focus 快捷键参考框，只展示 Global 与来源 `FocusKind` 的有效
...
Operations 的设计见 [`view-configuration-design.md`](view-configuration-design.md)。Schema
v1 已实现基础 View 栏宽、commit density/字段和 scoped operation bindings；更细的 panel
fields 与 presentation 配置仍按该文档继续演进。
