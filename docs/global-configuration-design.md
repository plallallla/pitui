...
struct CommandDescriptor {
    ...
    default_bindings: &'static [KeySequence],
    default_footer_label: &'static str,
...
`actionability` 仍由程序定义，并同时接收 `mode + screen + focus + selection + pending state +
...
因此当前 Commit 上下文中的 `Ctrl+C` 复制前缀与其他上下文中的 `Ctrl+C` 退出可以共存。
...
- 按下 `Ctrl+C` 后 footer 才切换为 `h hash | i info | m message | Esc cancel`；
...
4. root 中遇到 chord prefix 时只生成一个 group item，例如 `Ctrl+C copy…`；进入 prefix
   后才根据直接子节点生成 `h hash`、`i info`、`m message`。
...
弹窗正文中的按键说明也必须查询相同 keymap；安全保留键则查询固定的
`ReservedInteraction` 描述，不允许出现提示与真实操作不一致。
