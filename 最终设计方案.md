...
执行整文件 stage/unstage。commit 只提交当前 staged snapshot，并先进入提交消息 Context。
...
- Changes、Reflog、Remotes、Confirmation/Notice、GitOperationLog Dataset。
...
- Changes、Reflog、Remotes、Confirmation/Notice、GitOperationLog。
- `reflog` 仅 Command 入口。
...
13. Changes Dataset 可 stage/unstage/commit，且全局始终只有一个实例。
